...
const MovieShow = (props) => {
  ...
  const [filmsListArray, setFilmsListArray] = useState([]);
  ...
  useEffect(() => {
    ...
    setFilmsListArray(filmsListIdArray);
  }, []);

  const handleSubmit = () => {
    if (filmsListArray.includes(movieId)) {
  ...
  if (reviews?.length === 0) {
    trueReviewMessage = (
      <h4 className="message-text">No reviews have been added for this film. </h4>
    );
  } else {
  ...
  return (
    <div className="movie-tile">
      <div className="movie-tile-contents">
        ...
        <h4>
          {movie?.runtimeStr} - {movie?.contentRating}
        </h4>
